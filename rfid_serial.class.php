...
class phpSerial
{
	...
	function __construct ()
	{
		...
		{
			trigger_error("Host OS is neither linux nor windows, unable to run.", E_USER_ERROR);
			exit();
		}
	}
	...
	function setRaw ()
	{
		...
		if ($this->_os === "linux")
		{
			$ret = $this->_exec("stty -F " . $this->_device . " raw", $out);
		}

		if ($ret !== 0)
		{
			trigger_error ("Unable to set raw mode: " . $out[1], E_USER_WARNING);
			return false;
		}
	}
	...
	function confBaudRate ($rate)
	{
		...
		if (isset($validBauds[$rate]))
		{
			if ($this->_os === "linux")
			{
				$ret = $this->_exec("stty -F " . $this->_device . " " . (int) $rate . " raw" , $out);
			}
			elseif ($this->_os === "windows")
			{
			...
			if ($count !== 0)
			{
				do {
					if ($i > $count) $content .= fread($this->_dHandle, ($count - $i));
						...
						if ($content) trigger_error("Content Size ".strlen($content), E_USER_WARNING);
					}
